...
-----
判断一个数字是否对称
这题很好解，首先所有的负数是false,小于10的单数是true，剩下的大于10的就需要先转换为string，再针对string对半切分，判断两边是否对称。
...
class NumArray {
    ...
    public NumArray(int[] nums) {
        this.curSum = new int[nums.length];
        curSum[0] = nums[0];
        for (int i = 1; i < nums.length; i++)
    ...
    public int sumRange(int left, int right) {
        if (left == 0) return this.curSum[right];
        else
            return this.curSum[right] - this.curSum[left - 1];
    }
}
